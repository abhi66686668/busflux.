    ...
    function switchTab(tabId, el) {
      ...
      if (tabId === 'dashboard') {
        ...
        document.getElementById("linkTracking").classList.add("active");
      }
    ...
    let simCurrentStopIndex = 0;
    let simWaitTicks = 0;
    let simIsWaiting = false;
    ...
    function toggleJourney() {
      ...
      if (!journeyActive) {
        ...
        const stopCoordinates = {
          ...
          "deralakatte": [74.8982, 12.8090],
          "kuthar": [74.8698, 12.8158],
          "thokkottu": [74.8606, 12.8171],
          "nagori": [74.8745, 12.8700],
          "pumpwell": [74.8661, 12.8696],
    ...
    async function updateNextStop() {
      ...
      const busId = document.getElementById("liveBusSelect").value;
      const selectEl = document.getElementById("manualNextStop");
      const targetIndex = parseInt(selectEl.value, 10);
      if (isNaN(targetIndex) || !simAllStops.length) {
        alert("Please select a valid stop.");
        return;
      }
      
      const nextStopName = simAllStops[targetIndex];
      simCurrentStopIndex = targetIndex;
      simCurrentIndex = simStopIndices[simCurrentStopIndex];
      simIsWaiting = false;
      simWaitTicks = 0;
      
      conductorSocket.emit("conductor_location_update", {
         busId,
         lat: simCoords[simCurrentIndex][1],
         lng: simCoords[simCurrentIndex][0],
         routeCoordinates: simCoords,
         nextStop: `ARRIVED AT: ${nextStopName}`
      });
      
      document.getElementById("journeyStatus").textContent = `Status: Jumped to ${nextStopName}`;
      document.getElementById("nextStopBanner").textContent = `NEXT STOP: ${nextStopName}`;
      addNotification("Stop Jumped", `Teleported bus to: ${nextStopName}`, "success");
    }
