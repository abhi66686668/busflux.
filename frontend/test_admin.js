...
async function adminLogin() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      loadDashboard(); loadBuses(); loadUsers(); loadConductors(); loadBookings(); loadTransactions(); fetchSettlementsDashboard(); fetchAdminNotifications();
      const savedPage = sessionStorage.getItem("adminCurrentPage") || "dashboard";
      showPage(savedPage);
    } else {
...
function closeSettlementDetailsModal() {
  document.getElementById('settlementDetailsModal').classList.remove('active');
}
...
async function viewSettlementDetails(busId, ownerName) {
  ...
  document.getElementById('settlementDetailsModal').classList.add('active');
